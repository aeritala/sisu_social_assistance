
# setup ----
source(file = file.path("output",
                        "2024-01-09_weibull",
                        "constants.R"),
       local = TRUE)
prev_best <- readRDS(file.path(task_path,
                               "fit_weibull07_res.Rds"))
f <- prev_best$formula
h <- 5.1
hypotheses[[h]] <- str_replace(hypotheses[[5]], "PALKAT", "palkat1k_aik")

# fits ----
updates <- hypotheses[[5]][4:6]

writeSubModelTasks(task = task,
                   test_tag = paste0("h", h),
                   fit_tag = fit_tag,
                   dist = dist,
                   prior = "informative",
                   f = f,
                   update.f = updates,
                   warmup = 1500,
                   iter = 2500,
                   chains = 3,
                   seed = SEED,
                   overwrite = FALSE,
                   print = TRUE)

# analysis ----
writeAnalysisTasks(task = task,
                   fit_tag = fit_tag,
                   mini = FALSE,
                   overwrite = FALSE,
                   print = TRUE)

# report ----
n <- c(7:10, 14:16)
files <- file.path(task_path,
                   paste0("fit_", fit_tag, ifelse(n < 10,
                                                  paste0("0", n),
                                                  n),
                          "_res.Rds"))
suppressWarnings(rmarkdown::render(
  file.path("report",
            "functions",
            "summary_submodel_real.Rmd"),
  params = list(
    task = task,
    files = files
  ),
  output_format = "html_document",
  output_file = file.path("..", "..", task_path,
                          paste0("h", h, "_analysis_results.html")),
  knit_root_dir = getwd())
)


# fits ----

fit14 <- readRDS(file.path(task_path, "fit_weibull14.Rds"))
